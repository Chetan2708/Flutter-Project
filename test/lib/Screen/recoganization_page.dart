...
class _RecognizePageState extends State<RecognizePage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: _isLoad
            ...
            : EditableUserPage(
                name: userName, org: userOrg, txt: txt2, Date: date));
  }

  Future<void> analyzeEntitiesAPI(String rawData) async {
    final String ibmKey = dotenv.env['NATURAL_LANGUAGE_UNDERSTANDING_APIKEY'] ?? '';
     String url = dotenv.env['NATURAL_LANGUAGE_UNDERSTANDING_URL'] ?? '';
  ...
  String? fetchDate(String rawData) {
    ...
    List<RegExp> patterns = [
      ...
      exp5,
      RegExp(
          r'[A-Za-z]+,\s[A-Za-z]+\s\d{1,2},\s\d{4}'), // Tuesday, September 06, 2022
      RegExp(
          r'[A-Za-z]+,\s[A-Za-z]+\s\d{1,2},\s\d{2}'), // Tuesday, September 06, 22
